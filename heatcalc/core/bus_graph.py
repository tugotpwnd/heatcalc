...
def _detect_cross_tier_links(edges: Dict[int, Edge]) -> List[tuple[int, int]]:
    """
    Detect edges from different tiers that share a node.

    These represent copper that continues across a tier boundary.
    """
    links = []
...
def extract_graph(scene, px_to_m=0.001):
    ...
    cross_tier_links = _detect_cross_tier_links(edges)
